...
import { Path, PluginPath } from '../dir.js';
/**
 * 获取根路径
 * @param root - 根路径类型，可选 'root' | 'yunzai' | 'plugin'
 * @returns 对应根路径字符串
 */
function getRoot(root = 'plugin') {
    switch (root) {
        case 'root':
        case 'yunzai':
            return `${Path}/`;
 ...
 * @returns 解析后的绝对路径
 */
function resolvePath(file, root = 'plugin') {
...
export class Data {
     ...
     */
    static createDir(_path = '', root = 'plugin', includeFile = false) {
     ...
     */
    static getJSON(file, root = 'plugin', cache = true) {
        const filePath = resolvePath(file, root);
        if (cache && this.JSONCache.has(filePath))
     ...
     */
    static async readJSON(file, root = 'plugin') {
        const filePath = resolvePath(file, root);
        try {
     ...
     */
    static async writeJSON(file, data, root = 'plugin', space = 2) {
        const filePath = resolvePath(file, root);
        try {
     ...
     */
    static async setRedisJSON(key, data, EX = 3600 * 24 * 90) {
     ...
     */
    static async importModule(file, root = 'plugin') {
     ...
     */
    static async importDefault(file, root) {
        const mod = await this.importModule(file, root);
        return mod.default ?? {};
    }
    /**
     * 递归读取指定目录下所有匹配后缀的文件。
     ...
     */
    static readDirRecursive(directory, extension, excludeDir) {
     ...
     */
    static async asyncPool(limit, array, iteratorFn) {
        const results = [];
        const executing = [];
